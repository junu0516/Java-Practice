...
public class MemberController {
	...
	public void selectAll() {
		...
		List<Member> list = new MemberService().selectAll();
		if(!list.isEmpty()) {
	...
	public void selectOne(String memberId) {
		...
		Member m = new MemberService().selectOne(memberId); //Dao를 통해 DB에 접근하여 데이터를 리턴
	...
	public void insertMember(Member m) {
		// TODO Auto-generated method stub
		int result = new MemberService().insertMember(m); //insert, update, delete는 쿼리문의 성공 갯수를 리턴할 것
		if(result > 0) {
	...
	public void searchName(String memberName) {
		// TODO Auto-generated method stub
		MemberMenu menu = new MemberMenu();
		Member m = new MemberService().searchName(memberName);
		if(m!=null) {
			menu.displayMember(m);
		}
		else {
	...
	public void selectDeleted() {
		// TODO Auto-generated method stub
		MemberMenu menu = new MemberMenu();
		List<Member> listDeleted = new MemberService().selectDeleted();
		if(!listDeleted.isEmpty()) {
			menu.displayMemberList(listDeleted);
		} else {
			menu.displayError("해당되는 데이터가 없습니다.");
		}
	}
